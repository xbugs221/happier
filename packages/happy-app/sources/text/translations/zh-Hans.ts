...
export const zhHans: TranslationStructure = {
    ...
    proxyConfig: {
        ...
        statusEnabled: '已启用',
        statusDisabled: '未启用',
        currentHttpProxy: '当前 HTTP 代理',
        generalSettings: '基本设置',
        enableProxy: '启用代理',
        enableProxyDescription: '为此机器上启动的所有会话启用代理配置',
        securityWarning: '注意：代理配置包含敏感信息（可能包含用户名和密码）。请勿在不信任的设备上配置代理。',
        httpProxySettings: 'HTTP 代理设置',
        httpProxy: 'HTTP 代理',
        httpProxyPlaceholder: 'http://proxy.example.com:8080',
        httpsProxy: 'HTTPS 代理',
        httpsProxyPlaceholder: '留空则使用 HTTP 代理',
        advancedSettings: '高级设置',
        allProxy: 'All Proxy (SOCKS)',
        allProxyPlaceholder: 'socks5://127.0.0.1:1080',
        noProxy: 'No Proxy (排除列表)',
        noProxyPlaceholder: 'localhost,127.0.0.1,.local',
        saveSuccess: '代理配置已保存',
        saveFailed: '保存代理配置失败',
        errors: {
            required: '此字段为必填项',
            invalidScheme: '代理 URL 必须以 http://, https://, socks4://, socks5:// 或 socks:// 开头',
            missingHostname: '缺少主机名',
            invalidPort: '端口号必须在 1-65535 之间',
            invalidFormat: 'URL 格式无效',
            atLeastOneProxy: '启用代理时，至少需要配置 HTTP 代理或 All Proxy 中的一个',
        },
    },
